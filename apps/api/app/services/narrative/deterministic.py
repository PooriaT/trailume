...
class DeterministicNarrativeProvider:
    ...
    def generate(self, payload: NarrativeInput) -> NarrativeOutput:
        ...
        for card in payload.highlight_cards[:3]:
            highlights.append(f"{card.get('title', 'Highlight')}: {card.get('value', 'N/A')} ({card.get('detail', 'No detail')}).")
        ...
        if len(highlights) < 3:
            highlights.append(f"Total elevation gain: {float(total_elevation_m or 0):.0f} m.")
        ...
        if flags.get("hasRepeatedRouteTendency") and flags.get("repeatedRouteName"):
            reflection_parts.append(f"You repeatedly returned to {flags['repeatedRouteName']}, showing route consistency.")
